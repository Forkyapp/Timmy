...
Begin implementation now and make sure to create the PR when done!`;
  ...
  try {
    ...
    // Use `script` command to create a PTY session for Codex (it requires a TTY)
    const codexCommand = `script -q /dev/null bash -c 'cd "${repoPath}" && (echo "y"; sleep 2; cat "${promptFile}") | codex --yolo --sandbox danger-full-access'`;
    ...
    try {
      ...
      console.log(jarvis.error(`${colors.bright}Codex${colors.reset} execution failed: ${codexError.message}`));
...
Begin your review now and add TODO/FIXME comments to the code!`;
  ...
  try {
    ...
    try {
      ...
      console.log(jarvis.error(`${colors.bright}Codex${colors.reset} review execution failed: ${codexError.message}`));
