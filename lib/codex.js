...
function ensureCodexSettings(repoPath = null) {
...
async function launchCodex(task, options = {}) {
  ...
  const { analysis, subtask, branch, repoConfig } = options;
...
Begin implementation now and make sure to create the PR when done!`;
  ...
  try {
    ...
    const bashScript = `#!/bin/bash
      ...
      # Launch Codex in fully autonomous mode with prompt piped
...
`;
    ...
    // Execute script directly in background - Codex will open its own UI
    await execAsync(`/bin/bash '${scriptFile}' &`, {
      detached: true,
      stdio: 'ignore'
    });

    console.log(jarvis.success(`${colors.bright}Codex${colors.reset} active on ${colors.bright}task-${taskId}${colors.reset}`));

    await clickup.addComment(
      taskId,
      `🤖 **Codex Agent Deployed**\n\n` +
      `Codex is now executing this task autonomously.\n\n` +
      `**Branch:** \`task-${taskId}\`\n` +
...
Begin your review now and add TODO/FIXME comments to the code!`;
  ...
  try {
    ...
    const bashScript = `#!/bin/bash
      ...
      # Launch Codex for review in fully autonomous mode with prompt piped
      (echo "y"; sleep 2; cat "$PROMPT_FILE") | codex --yolo --sandbox danger-full-access
...
`;
    ...
    console.log(jarvis.success(`${colors.bright}Codex${colors.reset} review started for ${colors.bright}${branch}${colors.reset}`));

    await clickup.addComment(
      taskId,
      `👀 **Codex Code Review Started**\n\n` +
      `Codex is reviewing Claude's implementation and adding TODO/FIXME comments for improvements.\n\n` +
      `**Branch:** \`${branch}\`\n` +
