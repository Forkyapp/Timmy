...
# Launch Codex in fully autonomous mode with prompt
codex --yolo --sandbox danger-full-access < "${promptFile}"
...
`;
    ...
    if (isMac) {
      // macOS: Use osascript to open Terminal with the script
      await execAsync(`osascript -e 'tell application "Terminal" to do script "bash ${scriptFile}"'`);
    } else if (isWindows) {
      // Windows: Use start with cmd
      await execAsync(`start cmd /k "bash ${scriptFile}"`);
    } else {
      ...
      for (const term of terminals) {
        try {
          ...
          await execAsync(`${term} -e "bash ${scriptFile}"`);
          break;
        } catch {
          continue;
        }
...
# Launch Codex for review in fully autonomous mode with prompt
codex --yolo --sandbox danger-full-access < "${promptFile}"
