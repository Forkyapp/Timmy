    ...
    for (let i = prTrackingData.length - 1; i >= 0; i--) {
      ...
      if (result.found) {
        ...
        try {
          ...
          console.error(`Failed to add ClickUp comment:`, error.message);
        }

        try {
          await clickupModule.updateStatus(trackingEntry.taskId, 'can be checked');
        } catch (error) {
          console.error(`Failed to update ClickUp status:`, error.message);
        }
...
const pipeline = {
   ...
   */
  storeAgentExecution(taskId, agent, executionInfo) {
    const pipelines = this.load();
    const pipelineData = pipelines[taskId];
    ...
    if (!pipelineData.metadata.agentExecution) {
      pipelineData.metadata.agentExecution = {
        gemini: null,
        claude: null,
        codex: null
      };
    ...
    pipelineData.metadata.agentExecution[agent] = {
      ...executionInfo,
  ...
  /**
   * Get agent execution info for a task
   * @param {string} taskId - Task ID
   * @param {string} agent - Agent name (gemini, claude, codex) or null for all
   */
  getAgentExecution(taskId, agent = null) {
    const pipelineData = this.get(taskId);
    ...
    if (agent) {
      return pipelineData.metadata.agentExecution[agent];
    }

    return pipelineData.metadata.agentExecution;
  }
};
...
let processedCommentsSet = new Set();
...
const processedComments = {
  load() {
    try {
      ...
      console.error('Error loading processed comments:', error.message);
    }
  },
  ...
  save() {
    try {
      ...
      console.error('Error saving processed comments:', error.message);
    }
  },
